...
for segment in soup.find('div', {'class': 'flexible-content-body'}).find_all('p'):
	...
	# Zip into bigrams
	bigrams = [a.lower() + " " + b.lower() for a, b in zip(words, words[1:]) if a.lower() not in stop_words and b.lower() not in stop_words]

	# Add these bigrams to the frequency dictionary for this speaker
	freq = speakers.get(current_speaker, {})
	for bigram in bigrams:
		freq[bigram] = freq.get(bigram, 0) + 1
	speakers[current_speaker] = freq

# Find the top phrases used
